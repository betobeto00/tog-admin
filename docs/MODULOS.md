...
| 1 | **Productor** | ✅ MVP v1 (migración 036; gating por licencia y permisos, tests) | **Agricultura**: cultivos, siembras (área, cantidad sembrada, estado activa/cosechada), costos de campo por siembra y análisis de costos. Pendientes: estimación de cosecha y logística de acopio | Core |
| 2 | **Procesador** | ✅ MVP v1 (migraciones 049/050/051; en el binario vive bajo el módulo `productor`, con permisos `productor_cadenas_*` / `productor_lotes_*`) | **Cadenas de producción (BOM)**: producto final + pasos con insumos, costo override, mano de obra y overhead, precio recomendado. **Lotes**: crear (descuenta insumos), completar (ingresa stock final) y cancelar (devuelve insumos). **Tipo de producto** base/intermedio/final. Pendientes: mermas y recetas/BOM avanzadas | Core + Productor (opcional) |
...
| 4 | **Distribuidor** | ✅ v1 ampliado (migraciones 015/016, remitos y listas de precio; gating por licencia y permisos, tests) | Clientes (documento de registro internacional: RIF, RFC, EIN…, documento único entre activos, deuda pendiente en el listado), pedidos con estados y numeración, **remitos** con impresión A4 detallada (firmas de empresa y chofer) e **listas de precio** (factor global, override por producto, asignación por cliente). El **crédito a clientes vive en Comercializador** (migración 021): el POS vende fiado y valida `limite_credito` cuando se vincula a un cliente de este módulo. Pendientes: rutas, flotas y despachos | Core + Comercializador |
| 5 | **Postventa** | ✅ MVP v1 (migración 037; gating por licencia `postventa` + permisos `postventa_view` / `postventa_edit`, tests) | Tickets de soporte con numeración `PV-000001` y estados abierto/en proceso/resuelto/cerrado, devoluciones que **reponen stock** o quedan como nota de crédito, garantías con resolución (reponer/reembolsar/rechazar). Pendientes: SLA y adjuntos en tickets | Core + Comercializador |
| 6 | **Administración** | ✅ MVP v1 (migración 034, 5-Sep-2026; gating por licencia y permisos, tests) | **Submódulo Contable**: resumen ejecutivo del período, libro de ventas, libro de compras, libro de inventario con kardex, libro diario (asientos automáticos por venta/compra), export CSV para el contador. Pendientes: retenciones de ley por país, mayor y balance | Core + Comercializador |
| 7 | **Recursos Humanos** | ✅ v2 — nómina por capas (migración 054, 19-Sep-2026; gating por licencia y permisos, tests) | Empleados (CRUD + baja lógica), asistencia diaria (presente/tarde/ausente) con histórico filtrable e impresión A4, nómina por período con salario base prorrateado por tipo de pago, bonos y deducciones por empleado y globales, **catálogo de conceptos reutilizables**, **grupos de empleados con sus propios conceptos**, **vista previa antes de confirmar**, histórico de nómina por trabajador y recibos individuales/en lote. Pendientes: roles finos, beneficios legales por país | Core |
| 8 | **Restaurant** | ✅ MVP v1 (migración 024, gating por licencia y permisos, tests) | Mesas (CRUD + estado libre/ocupada), comanda por mesa (productos del catálogo con precio autocompletado + ítems manuales), pantalla de cocina (en preparación/listo/servido), **cobro de mesa** que factura solo ítems servidos/listos reutilizando `createVenta` (stock, combos, fiado, caja), e **impresión de comanda en la impresora de cocina** (`comandas:print-kitchen`, ESC/POS al puerto configurado) | Core + Comercializador |
| 9 | **Almacenes** | ✅ MVP v1 (migraciones 029/043; gating por permisos, tests) | Múltiples depósitos, stock y movimientos por almacén, caja asociada al almacén | Core + Comercializador |
| 10 | **Hípico** | ✅ MVP v1 (migraciones 047/048; gating por licencia `hipico` + permisos `hipico_*`, tests) | Propietarios, caballos, carreras (manuales o importadas de API pública), inscripciones con retiro, resultados con posición/tiempo/premio y **venta de apuestas**: cuotas, cobro, anulación, liquidación de premios, sincronización de odds y comprobante de apuesta. Las claves de las APIs de datos viven solo en el main | Core |
...
- **En producción**: TLS local con cert autofirmado generado y persistido al primer arranque de la Base (`services/red-cert.ts`; corregida su generación en el binario empaquetado el 19-Sep-2026) y heartbeat de 60 s para expulsar sesiones huérfanas (`useRedHeartbeat` + `barrerSesionesInactivas`). Detalle completo en `tog-platform/docs/INTERCONEXION-RED.md`.
...
- [x] Módulo Recursos Humanos — **MVP v1 (5-Sep-2026)**: empleados, asistencia, nómina por período con salario base prorrateado por tipo de pago, bonos/deducciones globales y por empleado, conceptos de nómina y pago. Pendiente: roles finos y beneficios legales por país.
- [x] Módulo Restaurant (mesas, comanda, cocina) — **MVP v1 (4-Sep-2026)**: ver `docs/historia/DISENO-MODULO-RESTAURANTE.md` y `docs/FEATURES.md` (RST1–RST5). Pendientes v2: cuentas divididas, enrutado de comandas a una impresora térmica dedicada (la impresión de comanda ya existe vía el flujo estándar), propinas, áreas del salón.
- [x] Interconexión por red local/Intranet entre PC Base y PC hijas (5-Sep-2026) — **spike funcional mergeado a master**: ver `docs/ARCHITECTURE.md` sección "Módulo Red Local" + `tog-platform/docs/INTERCONEXION-RED.md`. TLS local (cert autofirmado, `services/red-cert.ts`) y heartbeat 60 s (`useRedHeartbeat`) implementados el 5-Sep-2026.
