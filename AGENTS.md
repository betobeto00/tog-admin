...
- **Main process modularizado:** los handlers IPC se registran por módulo en `src/main/modules/<modulo>/` (inventario, ventas, configuracion, caja-extra, license, terminal, distribuidor, restaurant, administracion, rrhh, productor, postventa, hipico, print, **red**, crash-report, shared) y auth/usuarios en `src/main/core/auth/`. `src/main/ipc-handlers.ts` es solo el punto de registro que llama a cada `register*Handlers()`.
...
- **Catálogo de permisos:** `src/shared/permissions.ts` (única fuente; 69 permisos en 16 categorías; `ROLE_DEFAULTS.admin` = todas las claves).
...
- **Fechas: negocio en local, auditoría en UTC.** Las fechas de negocio (`fecha`, `fecha_apertura`, `fecha_cierre`, `cerrado_en`…) se insertan con `localDateStr()` / `localDateTimeStr()` (`src/main/utils/time.ts`, envoltorio de `datetime('now','localtime')`), **nunca** con `toISOString()` ni `datetime('now')`: la UI y los reportes comparan por día local y una venta de la noche caería en el día UTC siguiente (libro de ventas, flujo de efectivo y análisis financiero vacíos). Las columnas de auditoría (`creado_en`) se quedan en UTC.
- **Release / auto-update:** los assets del release deben llamarse como declara `latest.yml` (`TOG-Admin-Setup-<v>-x64.exe`, con **guiones**). Si se suben con el nombre que trae `release/` (espacios → GitHub los convierte en puntos) la URL del updater da **404** y la app no se puede actualizar. Ver `docs/UPDATER_NOTES.md`.
