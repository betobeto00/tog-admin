...
export default function MesasPage() {
  ...
  const sendKitchen = async () => {
    ...
    try {
      ...
      try {
        await callApi('comandas:print-kitchen', { comanda_id: comanda.id })
      } catch (printErr: any) {
        toast.warning(printErr?.message || t('restaurant.printError') || 'No se pudo imprimir en cocina')
      }
      await refreshComanda()
    } catch (err: any) {
      toast.error(err?.message || t('restaurant.error'))
  ...
  const printComanda = async () => {
    ...
    const rows = comanda.detalles
      ...
      .map((d) => `<tr><td style="text-align:center">${d.cantidad}</td><td>${escapeHtml(d.descripcion)}${d.notas ? ` <span style="color:#888">(${escapeHtml(d.notas)})</span>` : ''}</td></tr>`)
      .join('')
    abrirDocumento({
      titulo: `${t('restaurant.comandaTicket')} #${comanda.id}`,
      ancho: 400,
      alto: 700,
      estilos: `
      ...
      `,
      cuerpo: `
        <div class="center big">${escapeHtml(bizName || 'TOG Admin')}</div>
        ${bizAddr ? `<div class="center" style="font-size:9px">${escapeHtml(bizAddr)}</div>` : ''}
        ${bizPhone ? `<div class="center" style="font-size:9px">${escapeHtml(bizPhone)}</div>` : ''}
        <h2>${t('restaurant.comandaTicket')}</h2>
        <div class="center">${escapeHtml(comanda.mesa_nombre)} — #${comanda.id}</div>
  ...
  const printVentaTicket = async (ventaId: number) => {
    try {
      ...
      const rows = (venta.detalles || []).map((d: any) => {
        return `<tr><td style="text-align:center">${d.cantidad}</td><td>${escapeHtml(d.producto_nombre || d.descripcion || '')}</td><td style="text-align:right">${formatMoney(d.subtotal || 0)}</td></tr>`
      }).join('')
      abrirDocumento({
        titulo: `${t('ventas.ticket')} #${venta.numero_venta}`,
        ancho: 400,
        alto: 700,
        estilos: `
          body{font-family:monospace;font-size:11px;width:300px;margin:0 auto;padding:12px}
        ...
        cuerpo: `
          <div class="center big">${escapeHtml(bizName || 'TOG Admin')}</div>
          ${bizAddr ? `<div class="center small">${escapeHtml(bizAddr)}</div>` : ''}
          ${bizPhone ? `<div class="center small">${escapeHtml(bizPhone)}</div>` : ''}
          <h2>${t('ventas.ticket')} #${venta.numero_venta}</h2>
          <div class="center small">${formatDateTime(venta.fecha)}</div>
          ${venta.numero_control ? `<div class="center small">N° Control: ${escapeHtml(venta.numero_control)}</div>` : ''}
          ${venta.cliente_nombre ? `<hr><div><strong>${t('pos.invoiceClient') || 'Cliente'}:</strong> ${escapeHtml(venta.cliente_nombre)}</div>${venta.cliente_documento ? `<div class="small">${escapeHtml(venta.cliente_documento)}</div>` : ''}` : ''}
          <hr>
          <table>${rows}</table>
          <hr>
